...
// ── Imports SÍNCRONOS (Críticos para el arranque) ─────────────────────────────
...
// ── Imports LAZY (Carga perezosa para aligerar la app) ────────────────────────
...
// ── Layout Principal con Scroll Reset ────────────────────────────────────────
  ...
  return (
    <div className="relative min-h-screen bg-neutral-900 overflow-x-hidden flex flex-col">
      <div className="flex-1 flex flex-col overflow-hidden w-full" ref={tabRef}>
        <MotionDiv className="flex h-full" style={{ x }} drag="x" dragDirectionLock dragConstraints={{ left: -effectiveWidth * (TABS.length - 1), right: 0 }} dragElastic={0.06}
          onDragEnd={(_, info) => {
        ...
        >
          <div style={{ width: effectiveWidth }} className="shrink-0">
            <Dashboard user={session.user} habits={habits} todayLogs={todayLogs} onStartReview={() => navigate('/review')} version={CURRENT_SOFTWARE_VERSION} onOpenAdmin={() => navigate('/admin')} onOpenUpdates={() => setUpdateOpen(true)} hasUpdates={updateUnread} isTestAccount={isTestAccount} onResetTutorial={handleResetTutorial} onResetUpdates={handleResetUpdates} onOpenHistory={() => navigate('/history')} isPro={effectiveIsPro} isAdmin={isAdmin} onToggleTestPro={handleToggleTestPro} onUpgrade={() => setProModalOpen(true)} onResetToday={fetchTodayLogs} />
          </div>
          <div style={{ width: effectiveWidth }} className="shrink-0">
            <Suspense fallback={<RouteFallback />}><Stats user={session.user} isPro={effectiveIsPro} onUpgrade={() => setProModalOpen(true)} /></Suspense>
          </div>
          <div style={{ width: effectiveWidth }} className="shrink-0">
            <Suspense fallback={<RouteFallback />}><CommunityHub user={session.user} /></Suspense>
          </div>
          <div style={{ width: effectiveWidth }} className="shrink-0">
...
export default function App() {
  const navigate = useNavigate(); const location = useLocation(); const { t, language } = useLanguage()
  
  // 1. Cargamos sesión y el permiso Admin REAL de la base de datos (DB First)
  const { session, loadingSession, isAdmin, isBlocked, isWhitelisted, checkWhitelist } = useSession()

  // 2. Definimos si es cuenta de test (Vite env)
  const isTestAccount = useMemo(() => session?.user?.email === import.meta.env.VITE_TEST_EMAIL, [session])

  // 3. Unificamos el permiso (DB o Test) para la Torre de Control
  const effectiveIsAdmin = isAdmin || isTestAccount

  const { isMaintenance, maintenanceMessage, updateAvailable, updatePayload, updateUnread, setUpdateUnread, markUpdateSeen, resetUpdateSeen } = useAppSettings({ session, loadingSession, language })
  ...
  const [proModalOpen, setProModalOpen] = useState(false); const [updateOpen, setUpdateOpen] = useState(false)

  useEffect(() => {
    if (session && !session.user.user_metadata?.has_finished_tutorial && location.pathname !== '/tutorial') navigate('/tutorial', { replace: true })
  }, [session, navigate, location.pathname])

  if (loadingSession) return <div className="min-h-screen flex items-center justify-center bg-neutral-900 text-white font-black italic tracking-tighter uppercase text-3xl">DAYCLOSE</div>
  if (isMaintenance && !isWhitelisted && !effectiveIsAdmin) return <MaintenanceScreen message={maintenanceMessage} />
  if (!session) return (
    <Suspense fallback={<RouteFallback />}>
      <Routes>
  ...
  if (isBlocked && !effectiveIsAdmin) return <BlockedScreen title={t('blocked_title')} message={t('blocked_desc')} />
  ...
  return (
    <>
      ...
      <Routes>
        ...
        <Route path="/" element={<DashboardLayout session={session} habits={habits} todayLogs={todayLogs} effectiveIsPro={effectiveIsPro} isAdmin={effectiveIsAdmin} isTestAccount={isTestAccount} updateUnread={updateUnread} updateAvailable={updateAvailable} updateOpen={updateOpen} setUpdateOpen={setUpdateOpen} fetchTodayLogs={fetchTodayLogs} handleResetTutorial={async () => { await supabase.auth.updateUser({ data: { has_finished_tutorial: false } }); navigate('/tutorial', { replace: true }) }} handleResetUpdates={() => { resetUpdateSeen(updatePayload?.id); setUpdateOpen(true) }} handleToggleTestPro={handleToggleTestPro} setProModalOpen={setProModalOpen} t={t} />} />

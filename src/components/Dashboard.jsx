...
function CircularProgress({ percentage, completed, total }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div className={`relative flex h-48 w-48 items-center justify-center transition-all duration-500 ${isComplete ? "drop-shadow-[0_0_24px_rgba(16,185,129,0.4)]" : ""}`}>
        <svg className="h-full w-full -rotate-90 transform" viewBox="0 0 160 160">
          <circle cx="80" cy="80" r={radius} stroke="currentColor" strokeWidth="12" fill="none" className="text-neutral-800" />
          <circle cx="80" cy="80" r={radius} stroke="currentColor" strokeWidth="12" fill="none" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" className={`transition-all duration-700 ${isComplete ? "text-emerald-400" : "text-emerald-500"}`} />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          {isComplete ? (
            <>
              <span className="text-3xl mb-1">🎯</span>
              <p className="text-[10px] uppercase font-black text-emerald-400 tracking-[0.15em] px-2 text-center leading-tight">{t("all_done")}</p>
            </>
          ) : (
            <>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] uppercase tracking-widest text-neutral-600 font-bold ml-1">{t("habits")}</span>
      </div>
    </div>
  );
}
...
function Dashboard({
  ...
  useEffect(() => {
    try { localStorage.setItem("dayclose_hard_day_enabled", String(hardDayEnabled)) } catch {}
  }, [hardDayEnabled]);

  useEffect(() => {
    try { localStorage.setItem("dayclose_hard_day_ids", JSON.stringify(hardDayIds)) } catch {}
  }, [hardDayIds]);

  useEffect(() => {
    const validIds = new Set((habits || []).map((h) => h.id));
    const filtered = (hardDayIds || []).filter((id) => validIds.has(id));
    if (filtered.length !== hardDayIds.length) queueMicrotask(() => setHardDayIds(filtered));
  }, [habits, hardDayIds]);
  ...
  return (
    <div className="app-screen bg-neutral-900 px-4 relative">
      ...
      {isTestAccount && onToggleTestPro && (
        <button
          onClick={onToggleTestPro}
          className={`absolute top-6 right-4 z-[100] flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-black border transition-all ${isPro ? "bg-violet-500/20 border-violet-500/40 text-violet-300" : "bg-neutral-800 border-white/10 text-neutral-400"}`}
        >
          <Zap size={11} />
          {isPro ? "PRO" : "FREE"}
        </button>
      ...
      {/* isAdmin fluye: App (useSession RPC) → Dashboard → Sidebar */}
      <Sidebar
      ...
      <SettingsModal isOpen={isSettingsOpen} onClose={() => setSettingsOpen(false)} user={user} appVersion={version} isPro={isPro} onUpgrade={() => { setSettingsOpen(false); setProModalOpen(true) }} />
      <ProfileModal isOpen={isProfileOpen} onClose={() => setProfileOpen(false)} user={user} isPro={isPro} />
      <HabitCreator isOpen={isCreatorOpen || !!editHabit} onClose={() => { setCreatorOpen(false); setEditHabit(null) }} userId={user?.id} habitToEdit={editHabit} onHabitCreated={() => window.location.reload()} isPro={isPro} />
        ...
        {nearFreeLimit && (
          <button onClick={() => setProModalOpen(true)} className="mb-4 w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-amber-500/8 border border-amber-500/15 active:scale-98 transition-all group">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[11px] text-amber-300/80">{t("pro_soft_banner")}</p>
            </div>
            <span className="text-[10px] font-black text-amber-400/70 group-hover:text-amber-300 transition-colors whitespace-nowrap ml-2">{t("pro_soft_cta")}</span>
          </button>
        )}
        {atFreeLimit && (
          <div className="mb-6 flex items-center gap-3 px-4 py-3 rounded-2xl bg-violet-500/10 border border-violet-500/20">
            ...
            <p className="text-[12px] text-violet-300 flex-1">
              ...
              <button onClick={() => setProModalOpen(true)} className="underline font-bold">{t("pro_habit_limit_cta")}</button>{" "}
      ...
      <button onClick={() => setHardDayModalOpen(true)} className={`fixed bottom-52 right-6 h-12 w-12 rounded-[1rem] flex items-center justify-center transition-all z-40 ${hardDayEnabled ? "bg-white text-black shadow-2xl scale-110" : "bg-neutral-800/70 text-neutral-300 border border-white/5 shadow-apple-soft"}`}>
        <Frown size={20} />
      </button>

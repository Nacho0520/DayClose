...
import {
  ...
  Loader2, Lock, Zap, Award,
} from "lucide-react";
import { motion } from "framer-motion";
import { useLanguage } from "../context/LanguageContext";

// ── Utilidades de fecha — fuera del componente para evitar recreación ──────────

/** Formatea cualquier fecha a "YYYY-MM-DD" en hora local */
...
export default function Stats({ user, isPro, onUpgrade }) {
  ...
  // Estado raw de Supabase — solo datos sin procesar
  const [rawLogs, setRawLogs] = useState([]);
  const [rawAllLogs, setRawAllLogs] = useState([]);

  // Estado del protector de racha (localStorage)
  ...
  // ── Fetch de Supabase — solo carga datos, sin calcular nada ────────────────
  useEffect(() => {
    if (!user) return;
    ...
    async function fetchData() {
      ...
      // Query 1: Logs completados de los últimos 365 días.
      // Antes era ilimitada. Limitar a 365 días cubre cualquier racha
      // posible y reduce drásticamente el payload.
      const startStreak = new Date();
      startStreak.setDate(startStreak.getDate() - 364);
      ...
      const { data: logs, error: logsError } = await supabase
        ...
        .eq("user_id", user.id)
        .eq("status", "completed")
        .gte("created_at", startStreak.toISOString())
        .order("created_at", { ascending: false });

      if (logsError) {
        console.error("[Stats] Error cargando logs completados:", logsError.message);
        setLoading(false);
        return;
      }

      // Query 2: Todos los logs de los últimos 28 días (heatmap, sin filtro de status).
      const startHeatmap = new Date();
      startHeatmap.setDate(startHeatmap.getDate() - 27);
      ...
      if (allLogsError) {
        console.error("[Stats] Error cargando todos los logs (heatmap):", allLogsError.message);
      }

      setRawLogs(logs || []);
      setRawAllLogs(allLogs || []);
      setLoading(false);
    }
  ...
  // ── Todos los cálculos pesados en un único useMemo ─────────────────────────
  // Solo se recalcula cuando cambian los datos raw o el protector (no en re-renders de UI).
  const stats = useMemo(() => {
    if (!rawLogs.length && !rawAllLogs.length) {
      return {
        streak: 0,
        bestStreak: 0,
        totalCompleted: 0,
    ...
    // ── Días activos y protegidos ─────────────────────────────────────────
    const activeDays = new Set(rawLogs.map((log) => formatDate(log.created_at)));
    const protectedDays = new Set(protectorUses);
    ...
    // Calcular si se activa el protector automáticamente
    const monthKey = getMonthKey(today);
    const usesThisMonth = protectorUses.filter(
      (d) => typeof d === "string" && d.startsWith(monthKey)
    ).length;

    let nextProtectorUses = protectorUses;
    const eligible =
    ...
    if (eligible) {
      const merged = Array.from(new Set([...protectorUses, yesterdayStr]));
      protectedDays.add(yesterdayStr);
      nextProtectorUses = merged;
    }
    ...
    for (let i = 0; i < allDays.length; i++) {
      if (i === 0) {
        ...
        const diff =
          (new Date(allDays[i]) - new Date(allDays[i - 1])) / (1000 * 60 * 60 * 24);
        run = diff === 1 ? run + 1 : 1;
      }
    ...
    // Índice de activeDays por fecha para filtrado O(1)
    const weekDateSet = new Set(weekDates);
    const currentWeekData = weekDates.map((dateStr) => {
      const d = new Date(dateStr);
      const dayLogs = rawLogs.filter((l) => formatDate(l.created_at) === dateStr);
      return {
    ...
    const habitMap = {};
    rawLogs
      .filter((l) => weekDateSet.has(formatDate(l.created_at)))
      .forEach((l) => {
        const id = l.habit_id;
    ...
    for (let i = 27; i >= 0; i--) {
      ...
      heatmap.push({ date: dateStr, hasData, pct, isToday: dateStr === todayStr });
    }

    return {
      streak: currentStreak,
      bestStreak: best,
      totalCompleted: rawLogs.length,
      weeklyData: currentWeekData,
      weeklyHabitBreakdown: breakdown,
      weeklyRate: rate,
      heatmapData: heatmap,
      isProtectorActive: protectedDays.has(yesterdayStr),
      isPerfectWeek: perfectWeek,
      // Devolvemos nextProtectorUses para que el efecto secundario lo persista
      _nextProtectorUses: eligible ? nextProtectorUses : null,
    };
  }, [rawLogs, rawAllLogs, protectorUses]);

  // Efecto secundario: persistir el protector si fue activado automáticamente
  useEffect(() => {
    if (stats._nextProtectorUses) {
      try {
        localStorage.setItem(
          "dayclose_streak_protector_uses",
          JSON.stringify(stats._nextProtectorUses)
        );
      } catch {}
      setProtectorUses(stats._nextProtectorUses);
    }
  }, [stats._nextProtectorUses]);

  // ── Funciones de color puras (no crean closures en cada render) ────────────
  ...
  const {
    streak, bestStreak, totalCompleted,
    weeklyData, weeklyHabitBreakdown, weeklyRate,
    heatmapData, isProtectorActive, isPerfectWeek,
  } = stats;

        ...
        <form onSubmit={handleUpdate} className="space-y-4">
          <div className="bg-neutral-900/50 p-4 rounded-xl border border-neutral-700">
             ...
             <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => switchLanguage('es')} className={`py-2 px-3 rounded-lg text-xs font-bold transition-all border ${language === 'es' ? 'bg-blue-600 text-white border-blue-500' : 'bg-neutral-800 text-neutral-400 border-transparent hover:bg-neutral-700'}`}>🇪🇸 Español</button>
                <button type="button" onClick={() => switchLanguage('en')} className={`py-2 px-3 rounded-lg text-xs font-bold transition-all border ${language === 'en' ? 'bg-blue-600 text-white border-blue-500' : 'bg-neutral-800 text-neutral-400 border-transparent hover:bg-neutral-700'}`}>🇺🇸 English</button>
             </div>
          </div>
          <div>

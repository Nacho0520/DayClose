...
// ── Función de celebración con canvas-confetti ────────────────────────────────
// Disparo en dos oleadas desde los lados, igual que una app nativa premium
function launchConfetti() {
  ...
  // Oleada 1: desde la izquierda
  fire(0.25, { origin: { x: 0.2, y: 0.6 }, colors: ['#a855f7', '#6366f1', '#ffffff'] })
  // Oleada 2: desde la derecha
  fire(0.25, { origin: { x: 0.8, y: 0.6 }, colors: ['#10b981', '#34d399', '#ffffff'] })
  // Oleada 3: centro, ligeramente retrasada
  setTimeout(() => {
    fire(0.35, { origin: { x: 0.5, y: 0.5 }, colors: ['#f59e0b', '#ffffff', '#a855f7'] })
  }, 180)
  // Oleada 4: brillo final
  setTimeout(() => {
    fire(0.15, { origin: { x: 0.4, y: 0.3 }, colors: ['#ffffff', '#e2e8f0'] })
    fire(0.15, { origin: { x: 0.6, y: 0.3 }, colors: ['#ffffff', '#e2e8f0'] })
  }, 380)
}
...
export default function ReviewScreen({ habits, todayLogs, session, onReviewComplete }) {
  ...
  const { t } = useLanguage()
  const confettiFired = useRef(false) // evitar que confetti se dispare más de una vez

  // ── Hábitos pendientes de hoy (no logueados) ───────────────────────────
  const pendingHabits = useMemo(() => {
    const loggedIds = new Set((todayLogs || []).map(l => l.habit_id))
    return (habits || []).filter(h => !loggedIds.has(h.id))
  }, [habits, todayLogs])

  // ── Respeta el "Día difícil" configurado en localStorage ───────────────
  ...
  const allSwiped    = currentIndex >= reviewHabits.length && reviewHabits.length > 0

  // ── Disparar confetti cuando el usuario termina todos los swipes ───────
  // Se hace en la transición a la pantalla de score/mood, no al guardar
  useEffect(() => {
    if (allSwiped && !showDaySummary && !confettiFired.current) {
      confettiFired.current = true
      // Pequeño delay para que la animación de entrada de la tarjeta coincida
      setTimeout(launchConfetti, 300)
    }
  }, [allSwiped, showDaySummary])

  // ── Guardar resultados cuando se confirma el resumen ──────────────────
  const saveResults = useCallback(async () => {
    ...
    if (!error) {
      ...
      console.error('[ReviewScreen] Error guardando logs:', error.message)
    }
    setSaving(false)
  }, [session, results, hasSaved, saving, t, onReviewComplete, navigate])

  useEffect(() => {
    if (allSwiped && showDaySummary && !hasSaved && !saving) {
      saveResults()
    }
  }, [allSwiped, showDaySummary, hasSaved, saving, saveResults])

  // ── Loading mientras llegan los datos ─────────────────────────────────
  if (!habits) {
    return (
  ...
  return (
    ...
    >
          ...
          {currentHabit && (
            ...
            >
              <SwipeCard
                ...
                onDrag={(xVal) => setSwipeStatus(xVal > 100 ? 'done' : xVal < -100 ? 'not-done' : null)}
              />
            </motion.div>
          )}

          {/* ── Fase 2: Score y mood (con confetti ya disparado) ──────── */}
          {!currentHabit && !showDaySummary && (
            <MotionDiv
              key="summary-phase"
      ...
      <NoteModal
        ...
        onSave={(note) => {
          setResults(prev => [...prev, { id: pendingHabit.id, title: pendingHabit.title, status: 'skipped', note: note || '' }])
          setIsModalOpen(false)
          setCurrentIndex(c => c + 1)
        }}
        onSkip={() => {
          setResults(prev => [...prev, { id: pendingHabit.id, title: pendingHabit.title, status: 'skipped', note: '' }])
          setIsModalOpen(false)
          setCurrentIndex(c => c + 1)
        }}
      />

...
export default function ProgressComparison({ user }) {
  ...
  return (
    <div className="bg-neutral-800/30 radius-card border border-white/5 p-5">
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm font-semibold text-white">{t('compare_section_title')}</p>
        <span className="text-[10px] text-neutral-500 uppercase tracking-widest">{t('compare_section_desc')}</span>
      </div>
      <div className="grid grid-cols-3 gap-3">
        {['day', 'week', 'month'].map((key) => {
          ...
          return (
            <div key={key} className="bg-neutral-900/50 p-4 rounded-[1.5rem] border border-white/5 text-center">
              <p className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest">{t(`compare_${key}`)}</p>
              <p className="mt-1 text-xl font-black text-white">{data.current}</p>
              <p className={`text-[10px] font-bold uppercase tracking-widest ${isUp ? 'text-emerald-400' : 'text-red-400'}`}>

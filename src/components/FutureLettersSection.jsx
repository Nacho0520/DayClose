...
import { Mail, Clock, Zap, X, Send, Trash2 } from 'lucide-react'
...
export default function FutureLettersSection({ isPro }) {
  ...
  const orderedLetters = useMemo(() => [...letters].sort((a, b) => a.openAt - b.openAt), [letters])
  const now = Date.now()
  const readyLetters = orderedLetters.filter(l => l.openAt <= now)
  const daysLeft = orderedLetters[0] ? Math.max(0, Math.ceil((orderedLetters[0].openAt - now) / DAY_MS)) : null
  ...
  const handleOpenWriter = (e) => {
    e.preventDefault();
    e.stopPropagation();
    // Lógica de límite: Free solo 1 carta
  ...
  return (
    <div className="bg-neutral-900/40 p-5 rounded-2xl border border-white/5 relative overflow-hidden">
      ...
      {/* Header idéntico a "Más funciones" */}
      ...
      <div className="space-y-4">
        {/* Cuerpo de la sección - Visible para todos */}
          ...
          <div className="flex items-center gap-2 text-[10px] font-bold text-neutral-500 uppercase tracking-tight">
            ...
            {orderedLetters[0] ? (
              readyLetters.length > 0 ? (
                <span className="text-emerald-400 font-black">{t('more_letters_ready')}</span>
              ) : (
                <span>{daysLeft}d {t('remaining')}</span>
              )
            ) : (
      ...
      <AnimatePresence>
        {isLetterOpen && renderPortal(
          <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 pointer-events-auto">
            ...
            >
              <h3 className="text-white font-bold mb-1 text-base">{t('more_letters_modal_title')}</h3>
              <p className="text-[10px] text-neutral-500 uppercase font-black mb-4">{t('more_letters_modal_subtitle')}</p>
                ...
                {LETTER_DELAYS.map(d => (
                  <button 
                    ...
                    className={`flex-1 py-2 rounded-lg text-[10px] font-black border transition-all ${
                      letterDelay === d ? 'bg-white text-black border-white' : 'bg-neutral-800 text-neutral-400 border-white/5'
                    }`}
                  >
                    {d}d
                  </button>
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setIsLetterOpen(false)} className="flex-1 py-3 text-xs font-black text-neutral-500">{t('more_letters_cancel')}</button>
                <button 
                  type="button"
        ...
        {activeLetter && renderPortal(
          <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 pointer-events-auto">
            <MotionDiv 
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <h3 className="text-white font-bold text-base mb-4">{t('more_letters_open_title')}</h3>
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setActiveLetter(null)} className="flex-1 py-3 text-xs font-black text-neutral-500 bg-neutral-800 rounded-xl">{t('more_letters_close')}</button>
                <button 
                  type="button"
                  onClick={() => { persistLetters(letters.filter(l => l.id !== activeLetter.id)); setActiveLetter(null) }} 
